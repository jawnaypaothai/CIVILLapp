...
def analyze_crack(image_np):
    ...
    crack_pixel_count = np.count_nonzero(closed == 255)
    crack_area_mm2 = crack_pixel_count * (PIXEL_TO_MM ** 2)
    ...
    if crack_pixel_count < 1000:
        severity = "Light"
    elif crack_pixel_count < 5000:
        severity = "Moderate"
    else:
        severity = "Severe"

    return result, crack_pixel_count, crack_area_mm2, severity, orientations, avg_intensity, num_cracks
...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        processed, crack_pixel_count, area_mm2, severity, orientations, avg_intensity, num_cracks = analyze_crack(image_np)
        ...
        st.markdown(f"**จำนวนพิกเซลรอยร้าว:** {crack_pixel_count} px | **พื้นที่:** {area_mm2:.2f} มม² | **ทิศทาง:** {', '.join(translated_orientations)}")
